            ...
            for item in raw_predictions:
                ...
                conn.execute(
                    ...
                    INSERT INTO scan_predictions (
                        run_id, generated_at, selected_rank, is_selected, city_key, city_name, day_label, date_str,
                        event_slug, event_title, bucket, side, edge, ev_percent, price_cents, model_prob,
                        market_prob, ensemble_prediction, weighted_score, consensus_score, spread, sigma,
                        agreement_models, total_models, agreement_pct, confidence_tier, coverage_ok, degraded_reason,
                        policy_allowed, policy_reason, token_id, market_slug,
                        market_id, model_predictions_json, price_source, reference_price_cents, best_bid_cents,
                        polymarket_url
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        ...
                        generated_at,
                        selected_rank,
                        1 if selected_rank is not None else 0,
                        item["city_key"],
                        item["city_name"],
                        item["day_label"],
                        item["date_str"],
                        ...
                        item["spread"],
                        item["sigma"],
                        item.get("agreement_models", 0),
                        item.get("total_models", 0),
                        item.get("agreement_pct", 0.0),
                        item.get("confidence_tier", "risky"),
                        1 if item.get("coverage_ok") else 0,
                        item.get("degraded_reason"),
                        1 if item.get("policy_allowed") else 0,
                        item.get("policy_reason"),
                        item.get("token_id"),
                        item["market_slug"],
                        item["market_id"],
                        _json_dumps(item.get("model_predictions", {})),
                        item.get("price_source"),
                        item.get("reference_price_cents"),
                        item.get("best_bid_cents"),
                        item["polymarket_url"],
                    ),
                )
            ...
            for rank, item in enumerate(opportunities, start=1):
                conn.execute(
                    ...
                    INSERT INTO opportunities (
                        run_id, rank, city_key, city_name, day_label, date_str, event_slug, event_title,
                        bucket, side, edge, ev_percent, price_cents, model_prob, market_prob,
                        ensemble_prediction, weighted_score, consensus_score, spread, sigma,
                        token_id, market_slug, market_id, best_ask, last_trade_price, order_min_size,
                        agreement_models, total_models, agreement_pct, confidence_tier, coverage_ok, degraded_reason,
                        policy_allowed, policy_reason,
                        model_predictions_json, polymarket_url, price_source, reference_price_cents, best_bid_cents
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        run_id,
